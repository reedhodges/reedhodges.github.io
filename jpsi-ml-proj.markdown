...
<img src="https://raw.githubusercontent.com/reedhodges/reedhodges.github.io/main/images/expl_data_analysis.png" alt="PDFs Plot">
...
<p>In training an ML model, we will use more features than just these kinematic variables.</p>

<h2>Feature engineering</h2>

<p>To start with, we can the squares of the kinematic parameters x, z, Q, and PT to the features.  This could help capture some of the more complex functional dependence of the cross sections.  It is also worth looking at two ratios of these variables, which themselves are equal scalar products between momenta in the problem.</p>
...
<p>So far, the data we are dealing with is from theoretically-derived expressions, not the real world. Let's introduce some artificial imperfections to the data we have to make it seem more realistic.  Namely, we can destroy some of the data at random, as if some data was missing from the start, and add random noise.  This can be achieved with the following functions:</p>
...
<p>Modularizing the <code>mess_up_data</code> function lets us add more artificial imperfections later, if we so desire.  We can see what this does to what would otherwise be a smooth set of points generated by a continuous function.</p>

<img src="https://raw.githubusercontent.com/reedhodges/reedhodges.github.io/main/images/noise.png" alt="noise_fig">

<h2>Data cleaning</h2>

<p>Now we need to clean up the mess we made with the <code>mess_up_data</code> function.  For the destroyed data, we can replace the NaN values with the mean using an imputer.  We can also filter out outliers by removing the entries with a low z-score.</p>
...
<p>We are also interested in which production mechanism is dominant at each data point.  We can label the mechanisms that contribute to at least 30% of the total cross section at that point.</p>
...
<p>Many ML models will perform better if the data is scaled to a consistent value.  Let's use min/max scaling on our data.</p>
...
<p>All of the feature engineering, data "imperfectification", and data cleaning we have done can be summarized with the following data processing functions.</p>
